...
## 红线：禁止引导用户"粘贴 / 上传图片"到对话框

纯文本模型（DeepSeek 等）的请求链路**不接受图片内容块**：一旦对话中出现
`image` 类型的附件消息，每一轮模型请求都会立即失败（`UNSUPPORTED_CONTENT`），
且系统会自动把失败信息拼回对话并重开一轮，形成**无限失败循环**——会话卡死在
"运行中"状态、无法输入、持续消耗 API。**这是本 skill 最容易触发的故障，必须
从源头避免**：

- 用户要求看图时，一律引导其提供图片的**本地路径或 http(s) URL**，并说明
  "这样我能直接处理"（不要暴露"模型不支持图片"的底层细节）。
- 用户消息中出现**图片附件**（粘贴 / 拖拽 / 上传）时，不要尝试处理它——该
  附件不会到达模型，继续运行只会反复失败。礼貌地请用户改发图片的文件路径或
  网址（例："收到图片，但对话链路传不了图片字节，麻烦发一下图片的文件路径
  或网址，我马上处理"）。
- **绝不**回复"把图片粘贴到对话框里我就能看"之类的话。
- 若会话已经因图片附件卡死（一直显示运行中、发送无效）：请用户点击输入框旁
  的**停止按钮**中断当前轮次；若停止后仍自动重开失败轮次，**新开一个对话**
  继续（旧会话保持静止即可），并提醒用户今后只用路径 / URL 发图。

## 前提

- Node.js 18+（`node --version` 确认）
- 已配置 API Key。首次使用优先让用户运行仓库根的 `install.cmd`（cmd 直接输
  `install.cmd`，PowerShell 输 `.\install.cmd`，也可双击；一键安装 + 配置向导，
  除 API Key 外全程回车用免费默认）；已装好 skill 的则直接运行配置向导
  （`setup.cmd` 或 `setup.ps1`）：
...
- **图片是粘贴进对话的（不是路径/URL）**：纯文本模型无法获得图片字节，且图片附件
  会导致每轮请求直接失败、会话陷入失败重试循环（卡死）。礼貌地请用户提供图片的
  文件路径或 URL，说明这样模型才能"看"到它（不要暴露"模型不支持图片"的底层细节，
  用"路径方式更清晰"的说法即可）。**如果会话已经卡死**：点击停止按钮中断当前轮次；
  仍无法输入就新开一个对话，把图片路径 / URL 发过去即可。
